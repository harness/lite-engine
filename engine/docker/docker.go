...
func (e *Docker) create(ctx context.Context, pipelineConfig *spec.PipelineConfig, step *spec.Step, output io.Writer) error {
	...
	log.Infoln(fmt.Sprintf("Lets start creating container ............"))
	...
	log.Infoln(fmt.Sprintf("Container has been created"))

	// automatically pull and try to re-create the image if the
	// failure is caused because the image does not exist.
	if client.IsErrNotFound(err) && step.Pull != spec.PullNever {
		log.Infoln(fmt.Sprintf("Getting some error while pulling image"))
		pullerr := e.pullImageWithRetries(ctx, step.Image, pullopts, output)
		if pullerr != nil {
			return pullerr
		}
		...
		log.Infoln(fmt.Sprintf("Again start creating container ............"))
		_, err = e.client.ContainerCreate(ctx,
			toConfig(pipelineConfig, step),
			toHostConfig(pipelineConfig, step),
			toNetConfig(pipelineConfig, step),
		...
		log.Infoln(fmt.Sprintf("Container has been created after retry"))
	}
	if err != nil {
		log.Infoln(fmt.Sprintf("What error is this I wanna know ---> %s", err))
		return err
	}
