...
func TestSBT_GetCmd(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:                 "sbt -Duser.timezone=US/Mountain -Duser.locale=en/US 'set javaOptions ++= Seq(\"-javaagent:/install/dir/java/java-agent.jar=/test/tmp/config.ini\")' 'test'",
			expectedErr:          false,
		...
		},
		{
			...
			args:                 "clean test",
			runOnlySelectedTests: false,
			want:                 "sbt clean test 'set javaOptions ++= Seq(\"-javaagent:/install/dir/java/java-agent.jar=/test/tmp/config.ini\")' 'test'",
			expectedErr:          false,
			tests:                []ti.RunnableTest{},
		},
		{
			...
			want:                 "sbt clean test -Duser.timezone=US/Mountain -Duser.locale=en/US 'set javaOptions ++= Seq(\"-javaagent:/install/dir/java/java-agent.jar=/test/tmp/config.ini\")' 'testOnly pkg1.cls1 pkg2.cls2'",
			expectedErr:          false,
			tests:                []ti.RunnableTest{t1, t2},
		},
		{
		...
		},
		{
			...
			runOnlySelectedTests: true,
			want:                 "sbt clean test -B -2C-Duser.timezone=US/Mountain -Duser.locale=en/US 'set javaOptions ++= Seq(\"-javaagent:/install/dir/java/java-agent.jar=/test/tmp/config.ini\")' 'testOnly pkg1.cls1 pkg2.cls2'",
			expectedErr:          false,
		...
		},
		{
			...
			runOnlySelectedTests: true,
			want:                 "sbt clean test -B -2C -Duser.timezone=US/Mountain -Duser.locale=en/US || true 'set javaOptions ++= Seq(\"-javaagent:/install/dir/java/java-agent.jar=/test/tmp/config.ini\")' 'testOnly pkg2.cls2'",
			expectedErr:          false,
