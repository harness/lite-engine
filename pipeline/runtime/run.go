...
func executeRunStep(ctx context.Context, f RunFunc, r *api.StartStepRequest, out io.Writer, tiConfig *tiCfg.Cfg) ( //nolint:gocritic,gocyclo,funlen
	*runtime.State, map[string]string, map[string]string, []byte, []*api.OutputV2, string, error) {
	...
	log.Info(fmt.Sprintf("Here value of HARNESS_ACCOUNT_ID in step %s is --> %s ", step.Name, step.Envs["HARNESS_ACCOUNT_ID"]))
	log.Info(fmt.Sprintf("Here value of HARNESS_ACCOUNT_ID in step with apiStartstepRequest %s is --> %s ", step.Name, r.Envs["HARNESS_ACCOUNT_ID"]))
