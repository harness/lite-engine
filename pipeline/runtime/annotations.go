...
//
//nolint:funlen,gocyclo // The function orchestrates read/validate/fold/build/post steps; splitting would hurt readability.
func (e *StepExecutor) postAnnotationsToPipeline(ctx context.Context, r *api.StartStepRequest) {
	...
	// Prefer account identifier from file; fallback to step env: HARNESS_ACCOUNT_ID
	...
	if v := strings.TrimSpace(file.OrgID); v != "" {
		payload.OrgID = v
	} else if v := strings.TrimSpace(r.Envs["HARNESS_ORG_ID"]); v != "" {
		payload.OrgID = v
	}
	if v := strings.TrimSpace(file.ProjectID); v != "" {
		payload.ProjectID = v
	} else if v := strings.TrimSpace(r.Envs["HARNESS_PROJECT_ID"]); v != "" {
		payload.ProjectID = v
	}
	if v := strings.TrimSpace(file.PipelineID); v != "" {
		payload.PipelineID = v
	} else if v := strings.TrimSpace(r.Envs["HARNESS_PIPELINE_ID"]); v != "" {
		payload.PipelineID = v
	}
	...
	if err != nil || statusCode < 200 || statusCode >= 300 {
		if err != nil {
			logrus.WithFields(logrus.Fields{"id": r.ID, "url": fullURL, "attempt": 1}).WithError(err).Warnln("ANNOTATIONS: post failed")
		} else {
		...
		statusCode, respBody, err = client.PostJSON(ctx, endpoint, body)
		if err != nil {
			logrus.WithFields(logrus.Fields{"id": r.ID, "url": fullURL, "attempt": 2}).WithError(err).Warnln("ANNOTATIONS: post failed (retry)")
			return
		}
		if statusCode < 200 || statusCode >= 300 {
			logrus.WithFields(logrus.Fields{
				...
				"bytes":   len(respBody),
				"attempt": 2,
			}).Warnln("ANNOTATIONS: post non-success status (retry)")
			return
		}
	}
