...
func getPreCmd(workspace, tmpFilePath string, fs filesystem.FileSystem, log *logrus.Logger, envs, agentPaths map[string]string, isPsh bool, tiConfig *tiCfg.Cfg) (preCmd, filterFilePath string, err error) {
	...
	// Use DEBUG_MODE to redirect error logs as needed
	if !isPsh {
		preCmd = fmt.Sprintf(`if [ "$(printf %%s "$DEBUG_MODE" | tr '[:upper:]' '[:lower:]')" != "true" ]; then redir="2>/dev/null"; else redir=""; fi;`)
	}
	...
	if !isPsh {
		...
		preCmd += fmt.Sprintf(`\nif ($env:DEBUG_MODE -ieq 'true') { python3 -m pip install %s } else { try { python3 -m pip install %s 2>$null } catch { Write-Host 'Error: Failed to install Python agent.' } };`, whlFilePath, whlFilePath)
	}
