...
func SetupRunTestV2(ctx context.Context, config *api.RunTestsV2Config, stepID, workspace string, log *logrus.Logger, envs map[string]string, tiConfig *tiCfg.Cfg) (string, error) {
	...
	if config.IntelligenceMode {
		...
		err = downloadJavaAgent(ctx, tmpFilePath, links[0].URL, fs, log, tiConfig)
		if err != nil {
			return preCmd, fmt.Errorf("failed to download Java agent")
		}

		rubyArtifactDir, err := downloadRubyAgent(ctx, tmpFilePath, links[2].URL, fs, log, tiConfig)
		if err != nil || rubyArtifactDir == "" {
			return preCmd, fmt.Errorf("failed to download Ruby agent")
		}
		agentPaths["ruby"] = rubyArtifactDir

		pythonArtifactDir, err := downloadPythonAgent(ctx, tmpFilePath, links[1].URL, fs, log, tiConfig)
		if err != nil {
		...
		if len(links) > dotNetAgentLinkIndex {
			...
			dotNetArtifactDir, err = downloadDotNetAgent(ctx, tmpFilePath, links[dotNetAgentLinkIndex].URL, fs, log, tiConfig)
			if err == nil {
...
func downloadJavaAgent(ctx context.Context, path, javaAgentV2Url string, fs filesystem.FileSystem, log *logrus.Logger, tiConfig *tiCfg.Cfg) error {
	javaAgentPath := fmt.Sprintf("%s%s", javaAgentV2Path, javaAgentV2Jar)
	dir := filepath.Join(path, javaAgentPath)
	err := instrumentation.DownloadFile(ctx, dir, javaAgentV2Url, fs, tiConfig)
	if err != nil {
...
func downloadRubyAgent(ctx context.Context, path, rubyAgentV2Url string, fs filesystem.FileSystem, log *logrus.Logger, tiConfig *tiCfg.Cfg) (string, error) {
	dir := filepath.Join(path, "ruby", "ruby-agent.zip")
	installDir := filepath.Dir(dir)
	err := instrumentation.DownloadFile(ctx, dir, rubyAgentV2Url, fs, tiConfig)
	if err != nil {
...
func downloadPythonAgent(ctx context.Context, path, pythonAgentV2Url string, fs filesystem.FileSystem, log *logrus.Logger, tiConfig *tiCfg.Cfg) (string, error) {
	dir := filepath.Join(path, "python", "python-agent-v2.zip")
	installDir := filepath.Dir(dir)
	err := instrumentation.DownloadFile(ctx, dir, pythonAgentV2Url, fs, tiConfig)
	if err != nil {
...
func downloadDotNetAgent(ctx context.Context, path, dotNetAgentV2Url string, fs filesystem.FileSystem, log *logrus.Logger, tiConfig *tiCfg.Cfg) (string, error) {
	dotNetAgentPath := fmt.Sprintf("%s%s", dotNetAgentV2Path, dotNetAgentV2Zip)
	dir := filepath.Join(path, dotNetAgentPath)
	installDir := filepath.Dir(dir)
	err := instrumentation.DownloadFile(ctx, dir, dotNetAgentV2Url, fs, tiConfig)
	if err != nil {
