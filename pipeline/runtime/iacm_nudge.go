...
func getIACMNudges() []logstream.Nudge {
	return []logstream.Nudge{
		...
		logstream.NewNudge("[ERROR] Error processing environment variable",
			"Check the environment variables attached to the workspace. If the error persist, contact Harness Engineering",
			errors.New("there was an error starting the plugin")),
		logstream.NewNudge("[ERROR] Error setting environment variable",
			"Check the environment variables attached to the workspace. If the error persist, contact Harness Engineering",
			errors.New("there was an error starting the plugin")),
		logstream.NewNudge("[ERROR] Error processing Terraform variable",
			"Check the terraform variables attached to the workspace. If the error persist, contact Harness Engineering",
			errors.New("there was an error starting the plugin")),
		...
		logstream.NewNudge("[ERROR] Error creating the tfexec client for version",
			"Try changing the version of terraform in the workspace. If this is not possible of the error persist, contact Harness Engineering",
			errors.New("there was an error starting the plugin")),
		logstream.NewNudge("[ERROR] error finding Terraform binary",
			"Contact Harness Engineering",
			errors.New("there was an error starting the plugin")),
		...
		logstream.NewNudge("[ERROR] Error during the execution",
			"Check the logs for more information. If the problem persist, contact Harness Engineering",
		...
		logstream.NewNudge("[ERROR] Error initializing Terraform",
			"Error Initializing terraform. Please check the logs. If the problem persists, contact Harness Engineering",
			errors.New("error during the init step")),
		logstream.NewNudge("[ERROR] invalid Terraform plugin operation",
			"This operation is not supported. If you think that the operation should be supported ad is not please contact Harness Engineering",
			errors.New("error during the execution of the step. Invalid operation")),
		logstream.NewNudge("[ERROR] error during the apply phase",
			"Error during the apply step. Please check the logs. If the problem persists, contact Harness Engineering",
		...
		logstream.NewNudge("[ERROR] approval id is empty for the execution",
			"Please contact Harness Engineering",
			errors.New("error during the approval step")),
		logstream.NewNudge("[ERROR] Error executing Terraform plan",
			"Error generating terraform plan. Please check the logs. If the problem persists, contact Harness Engineering",
			errors.New("error during the plan step")),
		logstream.NewNudge("[ERROR] no plan available",
			"No terraform plan was found. Is the init step missing?. If the problem persist, contact Harness Engineering",
			errors.New("error during the plan step")),
		logstream.NewNudge("[ERROR] Error converting the Terraform plan to JSON",
			"The generated plan can't be converted to JSON. Check the tf files and if the problem persist, contact Harness Engineering",
			errors.New("error during the plan step")),
		logstream.NewNudge("[ERROR] no parsed plan available",
			"No terraform plan was found. Is the init step missing?. If the problem persist, contact Harness Engineering",
			errors.New("error during the plan step")),
		logstream.NewNudge("[ERROR] Error executing Terraform apply",
			"Error running terraform apply. Please check the logs. If the problem persists, contact Harness Engineering",
			errors.New("error during the apply step")),
		logstream.NewNudge("[ERROR] Error executing Terraform destroy",
			"Error running terraform destroy. Please check the logs. If the problem persists, contact Harness Engineering",
			errors.New("error during the destroy step")),
		logstream.NewNudge("[ERROR] no state file available",
			"No terraform state was found. Is the apply step missing?. If the problem persist, contact Harness Engineering",
			errors.New("error during the parsing step")),
		logstream.NewNudge("[ERROR] Error converting the Terraform state to JSON",
			"The generated state can't be converted to JSON. Check the tf files and if the problem persist, contact Harness Engineering",
			errors.New("error during the parsing step")),
		logstream.NewNudge("[ERROR] no parsed state file available",
			"No terraform state was found. Is the apply step missing?. If the problem persist, contact Harness Engineering",
			errors.New("error during the parsing step")),
		logstream.NewNudge("[ERROR] Error exporting output results",
			"There was an error parsing the outputs. Please check the logs and if the problem persist, contact Harness Engineering",
			errors.New("error generating output values")),
		logstream.NewNudge("[ERROR] error creating the gcloud credentials file",
			"Please check the logs. If the problem persists, contact Harness Engineering",
		...
		logstream.NewNudge("[ERROR] error creating sts creds with web identity token file",
			"Check if the serviceAccount has the proper IAM role attached to it. Please check the logs and if the problem persists, contact Harness Engineering",
			errors.New("error generating AWS credentials")),
		logstream.NewNudge("[ERROR] error creating sts creds by assuming the role",
			"Check if the IAM role used to assume the rol has the proper policies attached to it (assume role). Please check the logs and if the problem persists, contact Harness Engineering",
			errors.New("error generating AWS credentials")),
		...
		logstream.NewNudge("[ERROR] Error uploading Terraform data",
			"The communication with the IACM Server seems to be down. Please retry again and if the problem persists, contact Harness Engineering",
		...
		logstream.NewNudge("[ERROR] Error downloading state from the Terraform remote backend",
			"The communication with the terraform backend seems to be down. Check that the backend configuration is working and if the problem persists, contact Harness Engineering",
