...
func Test_CollectRunTestData(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			collectCgFn = func(ctx context.Context, stepID string, timeMs int64, log *logrus.Logger, start time.Time) error {
				return tc.cgErr
			}
			collectTestReportsFn = func(ctx context.Context, report api.TestReport, workDir, stepID string, log *logrus.Logger, start time.Time) error {
				return tc.crErr
			}
			err := collectRunTestData(ctx, log, &apiReq, time.Now(), stepName)
